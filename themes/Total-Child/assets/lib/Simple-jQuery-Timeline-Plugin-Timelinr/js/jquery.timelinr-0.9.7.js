    ...
    if (jQuery(settings.datesDiv).length > 0 && jQuery(settings.issuesDiv).length > 0) {
      ...
      jQuery(settings.datesDiv + ' a').click(function (event) {
        ...
        if (settings.orientation == 'horizontal') {
          ...
          jQuery(settings.issuesDiv).animate({'marginTop': -heightIssue * currentIndex}, {
            queue: false,
            duration: settings.issuesSpeed
          });
        }
        jQuery(settings.issuesDiv + ' li').animate({'opacity': settings.issuesTransparency}, {
          queue: false,
          duration: settings.issuesSpeed
        ...
        if (settings.orientation == 'horizontal') {
          jQuery(settings.datesDiv).animate({'marginLeft': defaultPositionDates - (widthDate * currentIndex)}, {
            queue: false,
            duration: 'settings.datesSpeed'
          });
        } else if (settings.orientation == 'vertical') {
          jQuery(settings.datesDiv).animate({'marginTop': defaultPositionDates - (heightDate * currentIndex)}, {
            queue: false,
            duration: 'settings.datesSpeed'
          });
        }
      ...
      jQuery(document).on('click', settings.nextButton, function (event) {
        event.preventDefault();
      ...
      jQuery(document).on('click', settings.prevButton, function (event) {
        event.preventDefault();
        // bugixed from 0.9.54: now the dates gets centered when there's too much dates.
        var currentIndex = jQuery(settings.issuesDiv).find('li.' + settings.issuesSelectedClass).index();
        if (settings.orientation == 'horizontal') {
