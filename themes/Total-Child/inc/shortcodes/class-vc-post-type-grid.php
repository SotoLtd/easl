...
if ( ! class_exists( 'EASL_Post_Type_Grid_Shortcode' ) ) {

	class EASL_Post_Type_Grid_Shortcode {
		...
		public function __construct() {
			...
			// Add shortcode
			add_shortcode( 'easl_post_type_grid', array( $this, 'output' ) );

			// Map to VC
			vc_lean_map( 'easl_post_type_grid', array( $this, 'map' ) );

			// Admin filters
			if ( is_admin() ) {

				// Get autocomplete suggestion
				add_filter( 'vc_autocomplete_easl_post_type_grid_tax_query_taxonomy_callback', 'vcex_suggest_taxonomies', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_filter_taxonomy_callback', 'vcex_suggest_taxonomies', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_tax_query_terms_callback', 'vcex_suggest_terms', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_author_in_callback', 'vcex_suggest_users', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_categories_taxonomy_callback', 'vcex_suggest_taxonomies', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_posts_in_callback', 'vc_include_field_search', 10, 1 );

				// Render autocomplete suggestions
				add_filter( 'vc_autocomplete_easl_post_type_grid_filter_taxonomy_render', 'vcex_render_taxonomies', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_tax_query_taxonomy_render', 'vcex_render_taxonomies', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_categories_taxonomy_render', 'vcex_render_taxonomies', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_tax_query_terms_render', 'vcex_render_terms', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_author_in_render', 'vcex_render_users', 10, 1 );
				add_filter( 'vc_autocomplete_easl_post_type_grid_posts_in_render', 'vc_include_field_render', 10, 1 );

				// Move content design elements into new entry CSS field
				add_filter( 'vc_edit_form_fields_attributes_easl_post_type_grid', 'vcex_parse_deprecated_grid_entry_content_css' );

				// Set image height to full if crop/width are empty
				add_filter( 'vc_edit_form_fields_attributes_easl_post_type_grid', 'vcex_parse_image_size' );
		...
		public function output( $atts, $content = null ) {
			ob_start();
			include( get_stylesheet_directory() . '/vc_templates/easl_post_type_grid.php' );
			return ob_get_clean();
		}
		...
		public function map() {
			return array(
				'name' => __( 'EASL Post Types Grid', 'total' ),
				...
				'params' => array(
					...
					array(
						...
						'description' => __( 'Add additional classes to the main element.', 'total' ),
						'param_name' => 'classes',
					),
					array(
...
new EASL_Post_Type_Grid_Shortcode;