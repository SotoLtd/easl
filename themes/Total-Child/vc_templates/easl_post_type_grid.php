...
$atts = vc_map_get_attributes( 'easl_post_type_grid', $atts );
...
if ( $wpex_query->have_posts() ) :
	...
	$wrap_classes       = array( 'vcex-module', 'easl-post-type-grid-wrap', 'wpex-clr' );
	$grid_classes       = array( 'wpex-row', 'easl-post-type-grid', 'entries', 'wpex-clr' );
	$grid_data          = array();
	$is_isotope         = false;
	...
	$output .= '<div class="'. $wrap_classes .'"'. vcex_get_unique_id( $unique_id ) .'>';
		...
		if ( 'true' == $entry_head ) {
			if ( $widget_title ) {
				...
				// Entry Classes
				$entry_classes   = array();
				$entry_classes[] = 'col span_1_of_1';
	//			$entry_classes   = array_merge( $static_entry_classes, $entry_classes );
				$output .= '<div class="' . implode( ' ', $entry_classes ) . '"">';
				...
				$output .= '<h2 class="wpb_easl_post_type_grid_head_title">' . $widget_title . '</h2>';
				if ( $widget_subtitle ) {
					$output .= '<p class="wpb_easl_post_type_grid_head_subtitle">' . $widget_subtitle . '</p>';
				}
				$output .= '</div>';

				if ( $view_all_url ) {
					$view_all_link = '<a class="wpb_easl_post_type_grid_head_all_link" href="' . esc_url( $view_all_url ) . '">' . $view_all_text . '</a>';
